...
function createRoomRoundManager({
    ...
    function resolveActiveCollisions(players) {
        const activePlayers = players.filter(
            (player) => !player.eliminated && !player.disconnected,
        );

        for (let index = 0; index < activePlayers.length; index += 1) {
            for (
                let comparisonIndex = index + 1;
                comparisonIndex < activePlayers.length;
                comparisonIndex += 1
            ) {
                resolveBallCollision(
                    activePlayers[index],
                    activePlayers[comparisonIndex],
                );
            }
        }
    }
    ...
    function updateSoloCpuInput(room) {
        ...
        const target = [...room.players.values()].find(
            (player) =>
                !player.isCpu && !player.eliminated && !player.disconnected,
        );
        ...
        const wobble =
            Math.sin((room.tickCount || 0) * config.wobbleFreq) *
            config.wobbleAmp;
