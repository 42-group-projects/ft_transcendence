      ...
      <section className="mx-auto w-full max-w-6xl space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight">Database Snapshot</h1>
          </div>
          <Link
            href="/lobby"
        ...
        <p className="text-sm text-neutral-400">Read-only preview from API endpoint: {DB_INSPECT_API_URL}</p>

        {loading ? <p className="text-neutral-300">Loading database contents...</p> : null}
        ...
        {!loading && !error && payload ? (
          <div className="space-y-4">
            <div className="rounded-xl border border-neutral-800 bg-neutral-900/80 p-4 text-sm text-neutral-300">
              ...
              <p>Max preview rows per table: {payload.maxPreviewRows}</p>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {sortedTables.map((table) => (
                ...
                >
                  <h2 className="text-base font-semibold text-neutral-100">{table.name}</h2>
                  <p className="mt-2 text-sm text-neutral-400">Rows: {table.rowCount}</p>
                  <p className="mt-1 text-xs text-neutral-500">Click to view table data</p>
                </Link>
              ))}
            </div>
          </div>
