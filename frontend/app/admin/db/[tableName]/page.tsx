...
const DB_INSPECT_API_URL = process.env.NEXT_PUBLIC_DB_INSPECT_API_URL ?? "http://localhost:4001/api/admin/db";
  ...
  useEffect(() => {
    ...
    const loadDbSnapshot = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error((data as { error?: string }).error ?? "Failed to fetch DB snapshot");
        }
  ...
  const filteredRows = useMemo(() => {
    ...
    return selectedTable.rows.filter((row) => {
      return columns.every((column) => {
        const query = (columnFilters[column] ?? "").trim().toLowerCase();
        if (!query) {
          return true;
        }

        return formatCellValue(row[column]).toLowerCase().includes(query);
      });
    });
    ...
    <main className="min-h-screen bg-neutral-950 px-4 py-6 text-neutral-100 sm:px-6 lg:px-8">
      <section className="mx-auto w-full max-w-7xl space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.3em] text-neutral-500">Admin</p>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight">Table: {tableName}</h1>
          </div>
          <Link
            href="/admin/db"
        ...
        {loading ? <p className="text-neutral-300">Loading table data...</p> : null}
        ...
        {!loading && !error && selectedTable ? (
          <article className="rounded-xl border border-neutral-800 bg-neutral-900/80 p-4">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-neutral-100">{selectedTable.name}</h2>
            ...
            <div className="mb-3 flex flex-wrap items-center gap-3 text-xs text-neutral-400">
              <span>
                Showing {filteredRows.length} / {selectedTable.rowCount} rows
              </span>
              <button
                type="button"
                onClick={() => {
                ...
                <div className="max-h-[70vh] overflow-auto">
                  <table className="w-full min-w-max border-collapse text-left text-xs text-neutral-200">
                    <thead className="sticky top-0 bg-neutral-900 text-neutral-300">
                      <tr>
                        {columns.map((column) => (
                          <th key={column} className="border-b border-neutral-800 px-3 py-2 font-medium uppercase tracking-wide">
                            {column}
                          </th>
                        ))}
                      </tr>
                      <tr>
                        {columns.map((column) => (
                          <th key={`${column}-filter`} className="border-b border-neutral-800 bg-neutral-900 px-3 py-2">
                            <input
                              type="text"
                    ...
                    <tbody>
                      {filteredRows.map((row, rowIndex) => (
                        <tr key={`${selectedTable.name}-${rowIndex}`} className="odd:bg-neutral-950 even:bg-neutral-900/40">
                          {columns.map((column) => (
                            <td key={`${selectedTable.name}-${rowIndex}-${column}`} className="border-b border-neutral-900 px-3 py-2 align-top font-mono">
