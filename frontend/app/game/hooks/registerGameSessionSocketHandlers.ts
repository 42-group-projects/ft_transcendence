...
export function registerGameSessionSocketHandlers({
  ...
  const onJoinedRoom = ({ roomId, playerId }: { roomId: string; playerId: string }) => {
  ...
  const onRoomError = ({ message }: { message: string }) => setErrorMessage(message);
  const onSystemMessage = ({ message }: { message: string }) => setSystemMessage(message);
  ...
  const onOpponentReconnected = () => setSystemMessage("Opponent reconnected.");
  const onCountdown = ({ secondsRemaining }: { secondsRemaining: number }) => {
    setCountdown(secondsRemaining);
    if (secondsRemaining === 0) setIsPaused(false);
  };
  const onSessionEnded = ({ reason, message }: { reason: string; message?: string }) => {
  ...
  const onResult = ({ message }: { message: string }) => setRoundResultMessage(message);
