...
import { apiGetMe, apiGetMyMatches, apiGetMyStats, type MatchRecord, type User, type UserStats } from "@/lib/api";
...
function MatchRow({ match, myId }: { match: MatchRecord; myId: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 rounded-lg border border-neutral-800 bg-neutral-950/60 px-4 py-3 text-sm">
      <div className="flex items-center gap-3 min-w-0">
        <span
          className={`inline-flex shrink-0 items-center rounded px-2 py-0.5 text-xs font-semibold ${
            won ? "bg-green-900/60 text-green-300" : "bg-red-900/60 text-red-400"
          }`}
        >
          {won ? "WIN" : "LOSS"}
        </span>
      ...
      <span className="shrink-0 text-neutral-500 tabular-nums">{timeAgo(match.played_at)}</span>
    </div>
  );
}
  ...
  return (
    <main className="min-h-screen bg-neutral-950 px-4 py-6 text-neutral-100 sm:px-6 lg:px-8">
      <section className="mx-auto w-full max-w-2xl">
        ...
        <h1 className="mt-3 text-3xl font-semibold tracking-tight">Match History</h1>
            ...
            {stats && (
              <div className="mt-6 grid grid-cols-3 gap-3 text-center">
                <div className="rounded-xl border border-neutral-800 bg-neutral-900 px-4 py-4">
                  ...
                  <p className="mt-1 text-xs text-neutral-500 uppercase tracking-wider">Wins</p>
                </div>
                <div className="rounded-xl border border-neutral-800 bg-neutral-900 px-4 py-4">
                  <p className="text-2xl font-bold text-red-400">{stats.losses}</p>
                  <p className="mt-1 text-xs text-neutral-500 uppercase tracking-wider">Losses</p>
                </div>
                <div className="rounded-xl border border-neutral-800 bg-neutral-900 px-4 py-4">
                  <p className="text-2xl font-bold text-blue-400">{stats.rating}</p>
                  <p className="mt-1 text-xs text-neutral-500 uppercase tracking-wider">Rating</p>
                </div>
              </div>
            )}
            ...
            <div className="mt-8">
              ...
              {matches.length === 0 ? (
                ...
                <div className="space-y-2">
                  {matches.map((m) => (
                    <MatchRow key={m.id} match={m} myId={user?.id ?? ""} />
                  ))}
                </div>
              )}
            </div>
