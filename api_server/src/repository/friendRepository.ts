...
export const friendRepository = {
    ...
    getPendingRequestBetweenUsers: async (
        db: AppDb,
        user1Id: string,
        user2Id: string,
    ) => {
    ...
    getFriendshipBetweenUsers: async (
        db: AppDb,
        user1Id: string,
        user2Id: string,
    ) => {
        const result = await db
            ...
            .where(
                and(
                    ...
                    or(
                        and(
                            eq(friendships.userId, user1Id),
                            eq(friendships.friendId, user2Id),
                        ),
                        and(
                            eq(friendships.userId, user2Id),
                            eq(friendships.friendId, user1Id),
                        ),
                    ),
                ),
            );
