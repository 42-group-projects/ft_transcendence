...
export const matchRepository = {
	 ...
	 */
	saveMatchResult: async (db: AppDb, input: MatchResultInput): Promise<void> => {
			...
			if (input.winnerId === input.player1Id) {
				await tx
					...
					.set({
						...
						...(input.isCpuGame ? {} : { rating: sql`${userStats.rating} + ${RATING_WIN_DELTA}` }),
						updatedAt: new Date(),
					})
				...
				await tx
					...
					.set({
						...
						...(input.isCpuGame ? {} : { rating: sql`GREATEST(0, ${userStats.rating} - ${RATING_LOSS_DELTA})` }),
						updatedAt: new Date(),
					})
					.where(eq(userStats.userId, input.player1Id));
			}
