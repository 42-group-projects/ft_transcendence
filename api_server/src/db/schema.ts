...
export const oauthProviderTypeEnum = pgEnum("oauth_provider_type", ["google", "github"]);
...
export const friendshipStatusEnum = pgEnum("friendship_status", ["accepted", "removed"]);

export const matchTypeEnum = pgEnum("match_type", ["keyword", "invite", "random", "cpu"]);

export const cpuLevelEnum = pgEnum("cpu_level", ["easy", "medium", "hard", "oni"]);
...
export const gameSessionStatusEnum = pgEnum("game_session_status", ["playing", "paused", "finished"]);
...
export const users = pgTable(
	...
	(table) => [
		...
		check("users_nickname_length_chk", sql`char_length(${table.nickname}) between 1 and 20`),
	]
);
...
export const oauthAccounts = pgTable(
	...
	(table) => [uniqueIndex("oauth_accounts_provider_user_uq").on(table.provider, table.providerUserId)]
);
...
export const friendRequests = pgTable(
	...
	(table) => [
		uniqueIndex("friend_requests_sender_receiver_uq").on(table.senderId, table.receiverId),
		check("friend_requests_not_self_chk", sql`${table.senderId} <> ${table.receiverId}`),
	]
);
...
export const friendships = pgTable(
	...
	(table) => [
		uniqueIndex("friendships_user_friend_uq").on(table.userId, table.friendId),
		check("friendships_not_self_chk", sql`${table.userId} <> ${table.friendId}`),
	]
);
...
export const gameRooms = pgTable(
	"game_rooms",
	{
		...
		guestId: uuid("guest_id").references(() => users.id, { onDelete: "restrict" }),
...
export const gameSessions = pgTable(
	"game_sessions",
	{
		...
		player1Id: uuid("player1_id")
			.notNull()
			.references(() => users.id, { onDelete: "restrict" }),
		player2Id: uuid("player2_id").references(() => users.id, { onDelete: "restrict" }),
		isCpuGame: boolean("is_cpu_game").notNull().default(false),
		cpuLevel: cpuLevelEnum("cpu_level"),
		winnerId: uuid("winner_id").references(() => users.id, { onDelete: "restrict" }),
	...
	(table) => [
		...
		check("game_sessions_finished_winner_chk", sql`${table.finishedAt} is null or ${table.winnerId} is not null`),
	]
);
...
export const usersRelations = relations(users, ({ many, one }) => ({
	...
	sentFriendRequests: many(friendRequests, { relationName: "friend_requests_sender" }),
	receivedFriendRequests: many(friendRequests, { relationName: "friend_requests_receiver" }),
	...
	gameSessionsAsPlayer1: many(gameSessions, { relationName: "game_sessions_player1" }),
	gameSessionsAsPlayer2: many(gameSessions, { relationName: "game_sessions_player2" }),
	wonGameSessions: many(gameSessions, { relationName: "game_sessions_winner" }),
	matchRecordsAsPlayer1: many(matchRecords, { relationName: "match_records_player1" }),
	matchRecordsAsPlayer2: many(matchRecords, { relationName: "match_records_player2" }),
	wonMatchRecords: many(matchRecords, { relationName: "match_records_winner" }),
	stats: one(userStats, {
		fields: [users.id],
