...
export async function getDbInspectSnapshot(maxPreviewRows = 100): Promise<DbInspectResult> {
  ...
  try {
    ...
    for (const tableName of tableNames) {
      ...
      const rows = await getTablePreviewRows(dbClient.db, tableName, maxPreviewRows);
